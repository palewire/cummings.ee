...
function instance($$self, $$props, $$invalidate) {
  ...
  Object.keys($$props).forEach((key) => {
    if (!~writable_props.indexOf(key) && key.slice(0, 2) !== '$$')
      console.warn(`<Poem> was created with unknown prop '${key}'`);
  });
...
class Poem extends SvelteComponentDev {
  constructor(options) {
    ...
    if (/*text*/ ctx[0] === undefined && !('text' in props)) {
      console.warn("<Poem> was created without expected prop 'text'");
    }
  }
...
async function main() {
  const url = new URL(
    new URL('assets/of-nicolette.txt', import.meta.url).href,
    import.meta.url
  );
